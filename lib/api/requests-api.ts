...
/**
 * Get requests with critic-agent in their agent chain
 * 
 * Returns requests with a confidenceOutputExists boolean indicating
 * whether confidence output already exists for each request.
 */
export async function getRequestsWithCriticAgent(): Promise<(RequestContext & { confidenceOutputExists: boolean })[]> {
  const response = await fetch(`${API_BASE}/with-critic-agent`)
