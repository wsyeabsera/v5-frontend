...
export function LocalPromptList() {
  const [viewDialogOpen, setViewDialogOpen] = useState(false)
  const [selectedPrompt, setSelectedPrompt] = useState<any>(null)

  const { data: prompts, isLoading, error } = usePrompts('local')
  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center p-12">
        ...
        <span className="ml-3 text-sm text-muted-foreground">Loading local prompts...</span>
      </div>
    )
  }
  ...
  if (error) {
    return (
      <div className="flex items-center gap-3 p-6 rounded-lg bg-red-50 dark:bg-red-950/20 border border-red-200 dark:border-red-900/30">
        ...
        <div>
          <div className="font-semibold text-red-900 dark:text-red-100">Failed to load local prompts</div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-2xl font-semibold">Local Prompts</h2>
        ...
        <div className="flex flex-col items-center justify-center p-12 rounded-lg bg-muted/30 border border-dashed border-border">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="text-center">
              <div className="font-semibold text-lg mb-1">No local prompts found</div>
              <div className="text-sm text-muted-foreground">
                No prompts available in the local database
              </div>
            </div>
          </div>
        </div>
