...
export function PlanFilters({
  ...
  const [thoughtIdInput, setThoughtIdInput] = useState(filters.thoughtId || '')
  ...
  const updateThoughtIdFilter = (value: string) => {
    setThoughtIdInput(value)
    onFiltersChange({
      ...filters,
      thoughtId: value.trim() || undefined,
  ...
  const updateStatusFilter = (value: string) => {
    onFiltersChange({
      ...filters,
      status: value === 'all' ? undefined : (value as 'pending' | 'in-progress' | 'completed' | 'failed'),
      skip: 0, // Reset pagination when filter changes
    })
  }
  ...
  const hasActiveFilters =
    (filters.thoughtId && filters.thoughtId.trim().length > 0) ||
    ...
    <div className="space-y-4 p-4 rounded-lg border bg-muted/30">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        ...
        <div className="space-y-2">
          <Label htmlFor="thoughtId-filter">Thought ID</Label>
          <Input
            id="thoughtId-filter"
            placeholder="Filter by thought ID..."
            value={thoughtIdInput}
            onChange={(e) => updateThoughtIdFilter(e.target.value)}
          />
        </div>
