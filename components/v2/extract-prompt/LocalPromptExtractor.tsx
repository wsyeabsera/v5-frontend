...
import { useExtractLocalPrompt } from '@/lib/queries-v2'
import { Loader2, AlertCircle, CheckCircle2 } from 'lucide-react'
import { ScrollArea } from '@/components/ui/scroll-area'

export function LocalPromptExtractor() {
  const [promptName, setPromptName] = useState('')
  const [argumentsJson, setArgumentsJson] = useState('')
  const [result, setResult] = useState<string | null>(null)
  const [error, setError] = useState<string | null>(null)

  const extractMutation = useExtractLocalPrompt()
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            The name of the prompt to extract from the local database
          </p>
        </div>

        <div className="space-y-2">
          <Label htmlFor="arguments">Arguments (JSON, optional)</Label>
          <Textarea
            id="arguments"
