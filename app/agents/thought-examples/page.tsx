...
export default function ThoughtExamplesPage() {
  const [examples, setExamples] = useState<ThoughtExample[]>([])
  const [isLoading, setIsLoading] = useState(true)
  const [isFormOpen, setIsFormOpen] = useState(false)
  const [isDeleteAllOpen, setIsDeleteAllOpen] = useState(false)
  const [editingExample, setEditingExample] = useState<ThoughtExample | null>(null)
  ...
  const loadExamples = async () => {
    ...
    setError(null)
    try {
      const response = await fetch('/api/thought-examples')
      if (!response.ok) {
  ...
  const handleEdit = (example: ThoughtExample) => {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      const response = await fetch(`/api/thought-examples?id=${id}`, {
        method: 'DELETE',
      })
  ...
  const handleSubmit = async (
    exampleData: Omit<ThoughtExample, 'id' | 'embedding' | 'createdAt' | 'updatedAt' | 'usageCount'>
  ) => {
    try {
      if (editingExample) {
        // Update existing
  ...
  const handleDeleteAll = async () => {
    try {
      const response = await fetch('/api/thought-examples?all=true', {
        method: 'DELETE',
      })
    ...
    <div className="min-h-screen p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="mb-1">Thought Examples</h1>
            <p className="text-[13px] text-muted-foreground">
              Manage example reasoning patterns for the Thought Agent
            </p>
          </div>
        ...
        <DeleteAllDialog
          ...
          title="Delete All Thought Examples"
          description="Are you sure you want to delete all thought examples? This will remove all examples from Pinecone and cannot be undone."
